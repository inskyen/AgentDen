...
如果接收端没有公网入站端口（agent 的云主机收不到 `@@514`），
可以用"拉"代替"推"：agent 侧 cron 每天一次 SSH 到目标机，
把 `/var/log/audit/audit.log` 整份拉回来，在**自己这边**解析比对。
日志离开机器的那一刻，接收端（agent 侧）就成立了。

诚实说明残留缺口：两次拉取之间，owner 若停审计、碰迷宫、再开审计，
这段真空拉取侧看不见。rsyslog 实时外发能堵上这个缺口（日志离机
是实时的），拉取式堵不上。v0 的威胁模型本来就不防处心积虑的 root，
选哪种如实写下来即可，不要把拉取式说成和实时外发一样。

另外：某些主机上 `ausearch` 会 hang 死不出结果（实测遇到过），
此时直接解析原始日志（`msg=audit(<epoch>.<sub>:<serial>)` 取时间戳过滤），
不要跟 ausearch 死磕。
